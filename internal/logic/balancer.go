...
func (lb *LoadBalancer) Update(ins []*naming.Instance) {
	...
	if len(ins) == 0 || float32(len(ins))/float32(len(lb.nodes)) < 0.5 {
		logger.Errorf("load balancer update src:%d target:%d less than half", len(lb.nodes), len(ins))
		return
	}
	lb.nodesMutex.Lock()
	for _, in := range ins {
		if old, ok := lb.nodes[in.Hostname]; ok && old.updated == in.LastTs {
			...
			if err != nil {
				logger.Errorf("instance(%+v) strconv.ParseInt(weight:%s) error(%v)", in, meta[model.MetaWeight], err)
				continue
			}
			conns, err := strconv.ParseInt(meta[model.MetaConnCount], 10, 32)
			if err != nil {
				logger.Errorf("instance(%+v) strconv.ParseInt(conns:%s) error(%v)", in, meta[model.MetaConnCount], err)
				continue
			}
