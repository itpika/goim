...
func InitWebsocket(server *Server, addrs []string, accept int) (err error) {
	...
	for _, bind = range addrs {
		...
		if listener, err = net.ListenTCP("tcp", addr); err != nil {
			logger.Errorf("net.ListenTCP(tcp, %s) error(%v)", bind, err)
			return
		}
		logger.Infof("start ws listen: %s", bind)
		// split N core accept
		for i := 0; i < accept; i++ {
...
func InitWebsocketWithTLS(server *Server, addrs []string, certFile, privateFile string, accept int) (err error) {
	...
	for i := range certFiles {
		...
		if err != nil {
			logger.Errorf("Error loading certificate. error(%v)", err)
			return
		}
	...
	for _, bind = range addrs {
		...
		logger.Infof("start wss listen: %s", bind)
		// split N core accept
		for i := 0; i < accept; i++ {
...
func acceptWebsocket(server *Server, lis *net.TCPListener) {
	...
	for {
		if conn, err = lis.AcceptTCP(); err != nil {
			...
			logger.Errorf("listener.Accept(%s) error(%v)", lis.Addr().String(), err)
			return
		}
		...
		if err = conn.SetReadBuffer(server.c.TCP.Rcvbuf); err != nil {
			logger.Errorf("conn.SetReadBuffer() error(%v)", err)
			return
		}
		if err = conn.SetWriteBuffer(server.c.TCP.Sndbuf); err != nil {
			logger.Errorf("conn.SetWriteBuffer() error(%v)", err)
			return
		}
...
func serveWebsocket(s *Server, conn net.Conn, r int) {
	...
	if conf.IsDebug() {
		...
		logger.Infof("start tcp serve \"%s\" with \"%s\"", lAddr, rAddr)
	}
...
func (s *Server) ServeWebsocket(conn net.Conn, rp, wp *bytes.Pool, tr *xtime.Timer) {
	...
	trd = tr.Add(time.Duration(s.c.Protocol.HandshakeTimeout), func() {
		...
		_ = conn.Close()
		logger.Errorf("key: %s remoteIP: %s step: %d ws handshake timeout", ch.Key, conn.RemoteAddr().String(), step)
	})
	// websocket
	...
	if req, err = websocket.ReadRequest(rr); err != nil || req.RequestURI != "/sub" {
		...
		rp.Put(rb)
		if err != io.EOF {
			logger.Errorf("http.ReadRequest(rr) error(%v)", err)
		}
		return
	}
	// writer
	...
	if ws, err = websocket.Upgrade(conn, rr, wr, req); err != nil {
		...
		wp.Put(wb)
		if err != io.EOF {
			logger.Errorf("websocket.NewServerConn error(%v)", err)
		}
		return
	}
	...
	if p, err = ch.CliProto.Set(); err == nil {
		if ch.Mid, ch.Key, rid, accepts, hb, err = s.authWebsocket(ctx, ws, p, req.Header.Get("Cookie")); err == nil {
			...
			err = b.Put(rid, ch)
			if conf.IsDebug() {
				logger.Infof("websocket connected key:%s mid:%d proto:%+v", ch.Key, ch.Mid, p)
			}
		}
	}
	step = 4
	if err != nil {
		...
		if err != io.EOF && err != websocket.ErrMessageClose {
			logger.Errorf("key: %s remoteIP: %s step: %d ws handshake failed error(%v)", ch.Key, conn.RemoteAddr().String(), step, err)
		}
		return
	}
	trd.Key = ch.Key
	...
	for {
		...
		if p.Op == protocol.OpHeartbeat {
			...
			if conf.IsDebug() {
				logger.Infof("websocket heartbeat receive key:%s, mid:%d", ch.Key, ch.Mid)
			}
			step++
		} else {
	...
	if err != nil && err != io.EOF && err != websocket.ErrMessageClose && !strings.Contains(err.Error(), "closed") {
		logger.Errorf("key: %s server ws failed error(%v)", ch.Key, err)
	}
	b.Del(ch)
	tr.Del(trd)
	ws.Close()
	...
	if err = s.Disconnect(ctx, ch.Mid, ch.Key); err != nil {
		logger.Errorf("key: %s operator do disconnect error(%v)", ch.Key, err)
	}
	if white {
		whitelist.Printf("key: %s disconnect error(%v)\n", ch.Key, err)
	}
	if conf.IsDebug() {
		logger.Infof("websocket disconnected key: %s mid:%d", ch.Key, ch.Mid)
	}
}
...
func (s *Server) dispatchWebsocket(ws *websocket.Conn, wp *bytes.Pool, wb *bytes.Buffer, ch *Channel) {
	...
	for {
		...
		if conf.IsDebug() {
			logger.Infof("key:%s dispatch msg:%s", ch.Key, p.Body)
		}
		switch p {
		case protocol.ProtoFinish:
			...
			if conf.IsDebug() {
				logger.Infof("key: %s wakeup exit dispatch goroutine", ch.Key)
			}
			finish = true
			goto failed
		...
		default:
			...
			if conf.IsDebug() {
				logger.Infof("websocket sent a message key:%s mid:%d proto:%+v", ch.Key, ch.Mid, p)
			}
		}
		if white {
...
failed:
	...
	if err != nil && err != io.EOF && err != websocket.ErrMessageClose {
		logger.Errorf("key: %s dispatch ws error(%v)", ch.Key, err)
	}
	ws.Close()
	wp.Put(wb)
	...
	if conf.IsDebug() {
		logger.Infof("key: %s dispatch goroutine exit", ch.Key)
	}
}
...
func (s *Server) authWebsocket(ctx context.Context, ws *websocket.Conn, p *protocol.Proto, cookie string) (mid int64, key, rid string, accepts []int32, hb time.Duration, err error) {
	for {
		...
		if p.Op == protocol.OpAuth {
			...
			logger.Errorf("ws request operation(%d) not auth", p.Op)
		}
	}
