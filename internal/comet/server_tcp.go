...
func InitTCP(server *Server, addrs []string, accept int) (err error) {
	...
	for _, bind = range addrs {
		...
		logger.Infof("start tcp listen: %s", bind)
		// split N core accept
		for i := 0; i < accept; i++ {
...
func (s *Server) ServeTCP(conn *net.TCPConn, rp, wp *bytes.Pool, tr *xtime.Timer) {
	...
	trd = tr.Add(time.Duration(s.c.Protocol.HandshakeTimeout), func() {
		conn.Close()
		logger.Errorf("key: %s remoteIP: %s step: %d tcp handshake timeout", ch.Key, conn.RemoteAddr().String(), step)
	})
	...
	if p, err = ch.CliProto.Set(); err == nil {
		if ch.Mid, ch.Key, rid, accepts, hb, err = s.authTCP(ctx, rr, wr, p); err == nil {
			...
			err = b.Put(rid, ch)
			if conf.IsDebug() {
				logger.Infof("tcp connnected key:%s mid:%d proto:%+v", ch.Key, ch.Mid, p)
			}
		}
	}
	step = 2
	if err != nil {
		...
		wp.Put(wb)
		tr.Del(trd)
		logger.Errorf("key: %s handshake failed error(%v)", ch.Key, err)
		return
	}
	trd.Key = ch.Key
	tr.Set(trd, hb)
	...
	if err != nil && err != io.EOF && !strings.Contains(err.Error(), "closed") {
		logger.Errorf("key: %s server tcp failed error(%v)", ch.Key, err)
	}
	b.Del(ch)
	tr.Del(trd)
	rp.Put(rb)
	...
	if err = s.Disconnect(ctx, ch.Mid, ch.Key); err != nil {
		logger.Errorf("key: %s mid: %d operator do disconnect error(%v)", ch.Key, ch.Mid, err)
	}
	if white {
...
failed:
	...
	if err != nil {
		logger.Errorf("key: %s dispatch tcp error(%v)", ch.Key, err)
	}
	conn.Close()
	wp.Put(wb)
...
func (s *Server) authTCP(ctx context.Context, rr *bufio.Reader, wr *bufio.Writer, p *protocol.Proto) (mid int64, key, rid string, accepts []int32, hb time.Duration, err error) {
	for {
		...
		if p.Op == protocol.OpAuth {
			...
			logger.Errorf("tcp request operation(%d) not auth", p.Op)
		}
	}
	if mid, key, rid, accepts, hb, err = s.Connect(ctx, p, ""); err != nil {
		logger.Errorf("authTCP.Connect(key:%v).err(%v)", key, err)
		return
	}
	p.Op = protocol.OpAuthReply
	p.Body = []byte(key)
	if err = p.WriteTCP(wr); err != nil {
		logger.Errorf("authTCP.WriteTCP(key:%v).err(%v)", key, err)
		return
	}
	p.Body = nil
	err = wr.Flush()
