...
func startClient(key int64) {
	...
	if err != nil {
		logger.Errorf("net.Dial(%s) error(%v)", os.Args[3], err)
		return
	}
	seq := int32(0)
	...
	if err = tcpWriteProto(wr, proto); err != nil {
		logger.Errorf("tcpWriteProto() error(%v)", err)
		return
	}
	if err = tcpReadProto(rd, proto); err != nil {
		logger.Errorf("tcpReadProto() error(%v)", err)
		return
	}
	logger.Infof("key:%d auth ok, proto: %v", key, proto)
	seq++
	// writer
	go func() {
		hbProto := new(Proto)
		for {
			...
			if err = tcpWriteProto(wr, hbProto); err != nil {
				logger.Errorf("key:%d tcpWriteProto() error(%v)", key, err)
				return
			}
			logger.Infof("key:%d Write heartbeat", key)
			time.Sleep(heart)
			seq++
			select {
			case <-quit:
	...
	for {
		if err = tcpReadProto(rd, proto); err != nil {
			logger.Errorf("key:%d tcpReadProto() error(%v)", key, err)
			quit <- true
			return
		}
		if proto.Operation == opAuthReply {
			logger.Infof("key:%d auth success", key)
		} else if proto.Operation == opHeartbeatReply {
			logger.Infof("key:%d receive heartbeat", key)
			...
			logger.Infof("key:%d op:%d msg: %s", key, proto.Operation, string(proto.Body))
